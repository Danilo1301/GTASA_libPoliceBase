...
std::vector<Vehicle*> Vehicles::GetAllCarsInSphere(CVector position, float radius)
{
    ...
    {
        auto vehiclePos = CleoFunctions::GetCarPosition(vehicle->hVehicle);
...
int Vehicles::GetClosestCar(CVector position, float radius)
{
    ...
    for(size_t i = 0; i < vehicles.size(); i++)
    {
        ...
        auto vehiclePosition = CleoFunctions::GetCarPosition(vehicle->hVehicle);
...
bool Vehicles::HasVehicleOfModelIdInArea(int modelId, CVector position, float radius)
{
    auto vehicles = Vehicles::GetAllCarsInSphere(position, radius);
